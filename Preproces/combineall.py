...
if __name__ == "__main__":
    ...
    sentiment_df = pd.read_csv("./daily_sentiment_summary.csv")
    ...
    merged_df_th = merge_stock_financial_sentiment(
        stock_filepath="./thai/Stock/stock_data_thai.csv",
        financial_filepath="./thai/Stock/Financial_Thai_Quarter.csv",
    ...
    merged_df_us = merge_stock_financial_sentiment(
        stock_filepath="./usa/Stock/stock_data_usa.csv",
        financial_filepath="./usa/Stock/Financial_America_Quarter.csv",
        sentiment_df=sentiment_df,
        country_name="usa"
    )
    
    # 5) รวมกันเป็นไฟล์เดียว
    merged_all = pd.concat([merged_df_th, merged_df_us], ignore_index=True)
    
    # 6) บันทึกเป็น CSV ไฟล์สุดท้าย
    merged_all.to_csv("merged_stock_sentiment_financial.csv", index=False)
