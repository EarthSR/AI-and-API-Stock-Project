...
df['Change (%)'] *= 100  # เปลี่ยนเป็นเปอร์เซ็นต์
...
# การเตรียมข้อมูล
feature_columns = ['Open', 'High', 'Low', 'Close', 'Volume', 'Change (%)', 'Sentiment', 'RSI', 'MACD', 'MACD_Signal', 'Bollinger_High', 'Bollinger_Low']
ticker_encoder = LabelEncoder()
df['Ticker_ID'] = ticker_encoder.fit_transform(df['Ticker'])
num_tickers = len(ticker_encoder.classes_)

# แบ่งข้อมูล Train/Test ตามเวลา
...
train_features_scaled = scaler_features.fit_transform(train_df[feature_columns].values)
test_features_scaled = scaler_features.transform(test_df[feature_columns].values)
