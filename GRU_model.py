...
trend_model = Sequential([
    GRU(64, activation='relu', return_sequences=True, input_shape=(X_trend.shape[1], X_trend.shape[2])),
    Dropout(0.2),
    GRU(32, activation='relu'),
    Dropout(0.2),
...
trend_model.save('trend_prediction_gru_model.h5')
...
for ticker in top_5_tickers:
    ...
    # แนวโน้ม
    trend_model = Sequential([
        GRU(64, activation='relu', return_sequences=True, input_shape=(X_ticker_trend.shape[1], X_ticker_trend.shape[2])),
        Dropout(0.2),
        GRU(32, activation='relu'),
        Dropout(0.2),
        Dense(1, activation='sigmoid')
